...
class CISD(ConfigurationInteraction):
    def __init__(self, system, **kwargs):
        ...
        # Number of singles states
        self.num_states = self.n * self.m + 1
        # Number of doubles states
        self.num_states += (
            self.n * (self.n - 1) // 2 * self.m * (self.m - 1) // 2
        )
    ...
    def setup_ci_space(self):
        ...
        index = create_singles_states(self.n, self.l, self.states, index=1)
        create_doubles_states(self.n, self.l, self.states, index=index)
        t1 = time.time()

        if self.verbose:
            print("Time spent setting up CISD space: {0} sec".format(t1 - t0))
